...
async function GroupUpdate(xyroo, m, store) {
	if (!m.messageStubType || !m.isGroup) return
	if (global.db?.groups?.[m.chat] && store?.groupMetadata?.[m.chat] && xyroo.public) {
		...
		if (global.db?.groups?.[m.chat]?.setinfo && messages[m.messageStubType]) {
			await xyroo.sendMessage(m.chat, { text: `${admin} ${messages[m.messageStubType]}`, mentions: [m.sender, ...(normalizedTarget?.includes('@') ? [`${normalizedTarget}`] : [])]}, { ephemeralExpiration: m.expiration || m?.metadata?.ephemeralDuration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		} else if (m.messageStubType == 20) {
			store.groupMetadata[m.chat] = await xyroo.groupMetadata(m.chat).catch(e => ({}));
			...
			if (m.fromMe && ((jidNormalizedUser(xyroo.user.id) == normalizedTarget) || (jidNormalizedUser(xyroo.user.lid) == normalizedTarget))) {
...
async function GroupParticipantsUpdate(xyroo, { id, participants, author, action }, store) {
	try {
		...
		if (global.db?.groups?.[id] && store?.groupMetadata?.[id]) {
			...
			for (let n of participants) {
				...
				let profile;
				try {
					profile = await xyroo.profilePictureUrl(n, 'image');
				} catch {
				...
				if (action === 'add') {
					...
					if ((jidNormalizedUser(xyroo.user.lid) == jidNormalizedUser(n)) || (jidNormalizedUser(xyroo.user.id) == jidNormalizedUser(n))) {
				...
				if (messageText && xyroo.public) {
					await xyroo.sendMessage(id, {
...
async function LoadDataBase(xyroo, m) {
	try {
		const botNumber = await xyroo.decodeJid(xyroo.user.id);
...
async function MessagesUpsert(xyroo, message, store) {
	try {
		let botNumber = await xyroo.decodeJid(xyroo.user.id);
		...
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) store.groupMetadata ??= await xyroo.groupFetchAllParticipating().catch(e => ({}));
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		const m = await Serialize(xyroo, msg, store)
		require('../xyroo')(xyroo, m, msg, store);
		if (db?.set?.[botNumber]?.readsw && msg.key.remoteJid === 'status@broadcast') {
			await xyroo.readMessages([msg.key]);
			if (/protocolMessage/i.test(type)) await xyroo.sendFromOwner(global.db?.set?.[botNumber]?.owner?.map(x => x.id) || global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
			if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
				...
				await xyroo.sendFromOwner(global.db?.set?.[botNumber]?.owner?.map(x => x.id) || global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
			}
		}
	} catch (e) {
		throw e;
...
async function Solving(xyroo, store) {
	xyroo.serializeM = (m) => MessagesUpsert(xyroo, m, store)
	...
	xyroo.getName = (jid, withoutContact  = false) => {
		const id = xyroo.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || (store.groupMetadata[id] ? store.groupMetadata[id] : (store.groupMetadata[id] = xyroo.groupMetadata(id))) || {};
	...
	xyroo.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await xyroo.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await xyroo.getName(i + '@s.whatsapp.net')}\nFN:${await xyroo.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await xyroo.getName(i + '@s.whatsapp.net')}\nFN:${await xyroo.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/xyroo_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		xyroo.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	
	xyroo.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await xyroo.query({
			tag: 'iq',
			attrs: {
	...
	xyroo.extractGroupMetadata = (result) => {
	...
	xyroo.groupMetadata = async (jid) => {
		...
		return xyroo.extractGroupMetadata(result);
	};
	
	xyroo.groupFetchAllParticipating = async () => {
		const result = await xyroo.query({ tag: 'iq', attrs: { to: '@g.us', xmlns: 'w:g2', type: 'get' }, content: [{ tag: 'participating', attrs: {}, content: [{ tag: 'participants', attrs: {}}, { tag: 'description', attrs: {}}]}]});
		const data = {};
		const groupsChild = getBinaryNodeChild(result, 'groups');
		if (groupsChild) {
			const groups = getBinaryNodeChildren(groupsChild, 'group');
			for (const groupNode of groups) {
				const meta = xyroo.extractGroupMetadata({
					tag: 'result',
					attrs: {},
					content: [groupNode]
				});
		...
		xyroo.ev.emit('groups.update', Object.values(data));
		return data;
	}

	xyroo.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return xyroo.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	}
	
	xyroo.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return xyroo.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, quotedOptions);
			} else if (mime && mime === 'application/pdf') {
				return xyroo.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('image')) {
				return xyroo.sendMessage(jid, { image: res.data, caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('video')) {
				return xyroo.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, quotedOptions);
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return xyroo.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('audio')) {
				return xyroo.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, quotedOptions);
			}
		}
	...
	xyroo.sendGroupInvite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
		...
		const invite = await xyroo.relayMessage(participant, message.message, { messageId: message.key.id })
		return invite
	}
	
	xyroo.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await xyroo.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
	}
	
	xyroo.sendText = async (jid, text, quoted, options = {}) => xyroo.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	
	xyroo.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return xyroo.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	
	xyroo.downloadMediaMessage = async (message) => {
	...
	xyroo.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		const buffer = await xyroo.downloadMediaMessage(message);
	...
	xyroo.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: xyroo.user.id, quoted: m.quoted && m.quoted.fakeObj, ephemeralExpiration: m.expiration || m?.metadata?.ephemeralDuration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
		apb.key = m.key
		apb.key.id = [...Array(32)].map(() => '0123456789ABCDEF'[Math.floor(Math.random() * 16)]).join('');
		apb.key.fromMe = areJidsSameUser(m.sender, xyroo.user.id);
		if (m.isGroup) apb.participant = m.sender;
		xyroo.ev.emit('messages.upsert', {
			...m,
	...
	xyroo.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await xyroo.getFile(path, true);
		...
		if (isWebpSticker) {
			pathFile = await writeExif(data, {
				...
				author: options.author || db?.set?.[botNumber]?.author || 'Xyroodev',
		...
		let anu = await xyroo.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0, ...options });
		await fs.unlinkSync(pathFile);
		return anu;
	}
	
	xyroo.sendAlbumMessage = async (jid, content = {}, options = {}) => {
		...
		await xyroo.relayMessage(jid, medias.message, { messageId: medias.key.id });
		for (const media of album) {
			const msg = await generateWAMessage(jid, { ...others, ...media }, { upload: xyroo.waUploadToServer });
			...
			await xyroo.relayMessage(jid, msg.message, { messageId: msg.key.id });
		}
		return medias;
	}
	
	xyroo.sendListMsg = async (jid, content = {}, options = {}) => {
		...
		const hasil = await xyroo.relayMessage(msg.key.remoteJid, msg.message, {
	...
	xyroo.sendButtonMsg = async (jid, content = {}, options = {}) => {
	...
	xyroo.newsletterMsg = async (key, content = {}, timeout = 5000) => {
		...
		if (react) {
			...
			const msg = await generateWAMessageContent(media, { upload: xyroo.waUploadToServer });
			const anu = await xyroo.query({
				tag: 'message',
			...
			const _query = await xyroo.query({
				tag: 'iq',
				attrs: {
					to: 's.whatsapp.net',
					type: 'get',
	...
	xyroo.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: xyroo.waUploadToServer });
			return imageMessage;
		}
		...
		const hasil = await xyroo.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	if (xyroo.user && xyroo.user.id) {
		const botNumber = xyroo.decodeJid(xyroo.user.id);
		if (global.db?.set[botNumber]) {
			xyroo.public = global.db.set[botNumber].public
		} else xyroo.public = true
	} else xyroo.public = true
...
async function Serialize(xyroo, msg, store) {
	const botLid = xyroo.decodeJid(xyroo.user.lid);
	const botNumber = xyroo.decodeJid(xyroo.user.id);
	const m = { ...msg };
	if (!m) return m
	if (m.key) {
		...
		if (!m.isGroup && m.chat.endsWith('@lid')) m.chat = xyroo.findJidByLid(m.chat, store) || m.chat;
		m.sender = xyroo.decodeJid(m.fromMe && xyroo.user.id || m.key.participant || m.chat || '')
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await xyroo.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = await xyroo.groupMetadata(m.chat).catch(e => ({})))
			if (!metadata) {
				metadata = await xyroo.groupMetadata(m.chat).catch(e => ({}))
			...
			if (metadata.addressingMode === 'lid') {
				...
				store.contacts[m.sender] = { ...store.contacts[m.sender], id: m.sender, lid: m.fromMe && xyroo.user.lid || participant?.lid || m.sender, name: m.pushName };
			}
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = xyroo.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === xyroo.decodeJid(xyroo.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, xyroo)
				return await Serialize(xyroo, q, store)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(xyroo.decodeJid(m.msg?.contextInfo?.participant), xyroo.decodeJid(xyroo?.user?.id)),
			...
			m.quoted.download = () => xyroo.downloadMediaMessage(m.quoted)
			m.quoted.delete = () => {
				xyroo.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.download = () => xyroo.downloadMediaMessage(m)
	
	m.copy = () => Serialize(xyroo, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
	
	m.react = (u) => xyroo.sendMessage(m.chat, { react: { text: u, key: m.key }})
	...
	m.reply = async (content, options = {}) => {
		...
		if (typeof content === 'object') {
			return xyroo.sendMessage(chat, content, { ...options, quoted, ephemeralExpiration })
			...
			try {
				if (/^https?:\/\//.test(content)) {
					...
					if (/gif|image|video|audio|pdf|stream/i.test(mime)) {
						return xyroo.sendMedia(chat, data.data, '', caption, quoted, content)
					} else {
						return xyroo.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
					}
				} else {
					return xyroo.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
				}
