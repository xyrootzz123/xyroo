...
async function startXyrooBot() {
	const { state, saveCreds } = await useMultiFileAuthState('xyroodev');
	...
	const getMessage = async (key) => {
		...
		return {
			conversation: 'Halo Saya Xyroo Bot'
		}
	}
	
	const xyroo = WAConnection({
		logger: level,
		getMessage,
	...
	if (pairingCode && !phoneNumber && !xyroo.authState.creds.registered) {
		...
		(async () => {
			await getPhoneNumber();
			await exec('rm -rf ./xyroodev/*');
			console.log('Phone number captured. Waiting for Connection...\n' + chalk.blueBright('Estimated time: around 2 ~ 5 minutes'))
		})()
	}
	
	await Solving(xyroo, store)
	
	xyroo.ev.on('creds.update', saveCreds)
	
	xyroo.ev.on('connection.update', async (update) => {
		const { qr, connection, lastDisconnect, isNewLogin, receivedPendingNotifications } = update
		if (!xyroo.authState.creds.registered) console.log('Connection: ', connection || false);
		if ((connection === 'connecting' || !!qr) && pairingCode && phoneNumber && !xyroo.authState.creds.registered && !pairingStarted) {
			setTimeout(async () => {
				pairingStarted = true;
				console.log('Requesting Pairing Code...')
				let code = await xyroo.requestPairingCode(phoneNumber);
		...
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.connectionLost) {
				...
				xyroo.end(`Unknown DisconnectReason : ${reason}|${connection}`)
			}
		}
		if (connection == 'open') {
			console.log('Connected to : ' + JSON.stringify(xyroo.user, null, 2));
			let botNumber = await xyroo.decodeJid(xyroo.user.id);
			if (global.db?.set[botNumber] && !global.db?.set[botNumber]?.join) {
				if (my.ch.length > 0 && my.ch.includes('@newsletter')) {
					if (my.ch) await xyroo.newsletterMsg(my.ch, { type: 'follow' }).catch(e => {})
	...
	xyroo.ev.on('contacts.update', (update) => {
		for (let contact of update) {
			let trueJid;
			if (!trueJid) continue;
			if (contact.id.endsWith('@lid')) {
				trueJid = xyroo.findJidByLid(contact.id, store);
			} else {
	...
	xyroo.ev.on('call', async (call) => {
		let botNumber = await xyroo.decodeJid(xyroo.user.id);
		if (global.db?.set[botNumber]?.anticall) {
			for (let id of call) {
				if (id.status === 'offer') {
					let msg = await xyroo.sendMessage(id.from, { text: `Saat Ini, Kami Tidak Dapat Menerima Panggilan ${id.isVideo ? 'Video' : 'Suara'}.\nJika @${id.from.split('@')[0]} Memerlukan Bantuan, Silakan Hubungi Owner :)`, mentions: [id.from]});
					await xyroo.sendContact(id.from, global.owner, msg);
					await xyroo.rejectCall(id.id, id.from)
				}
			}
		}
	});
	
	xyroo.ev.on('messages.upsert', async (message) => {
		await MessagesUpsert(xyroo, message, store);
	});
	
	xyroo.ev.on('group-participants.update', async (update) => {
		await GroupParticipantsUpdate(xyroo, update, store);
	});
	
	xyroo.ev.on('groups.update', (update) => {
	...
	xyroo.ev.on('presence.update', ({ id, presences: update }) => {
		store.presences[id] = store.presences?.[id] || {};
		Object.assign(store.presences[id], update);
	});
	
	setInterval(async () => {
		if (xyroo?.user?.id) await xyroo.sendPresenceUpdate('available', xyroo.decodeJid(xyroo.user.id)).catch(e => {})
	}, 10 * 60 * 1000);
